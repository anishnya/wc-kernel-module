...
for config in "${!FLUSH_CONFIGS[@]}"; do
    ...
    mkdir -p "${SCRIPT_DIR}/../results_v2/cached_${desc}"
    
    echo "Running with temporal_flush=${temporal}, non_temporal_flush=${non_temporal} (${desc})"
    "${SCRIPT_DIR}/../test/wc_test" cached "${SCRIPT_DIR}/dev" "${temporal}" "${non_temporal}" > "${SCRIPT_DIR}/out_${desc}.txt"
    ...
    mv "${SCRIPT_DIR}/"*.txt "${SCRIPT_DIR}/../results_v2/cached_${desc}/"

    echo "Benchmark completed successfully"
    echo "Results saved in: ${SCRIPT_DIR}/../results_v2/cached_${desc}/"
done
