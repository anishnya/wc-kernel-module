...
void producer(SharedData *data, std::size_t buffer_size, int iterations, int thread_id)
{
    // Pin to core 3
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
...
void consumer(SharedData *data, std::size_t buffer_size, int thread_id)
{
    // Pin to core 4
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
...
void producer_avx512(SharedData *data, std::size_t buffer_size, int iterations, int thread_id)
{
    // Pin to core 3
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
...
void consumer_avx512(SharedData *data, std::size_t buffer_size, int thread_id)
{
    // Pin to core 4
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
                     ...
                     int num_threads)
{
    if (num_threads == 1)
    {
        producer_avx512(data_ptrs[0], buffer_size, iterations, 0);
        return;
    }
    ...
    {
        // Create and store thread in vector
        threads.emplace_back(producer, data_ptrs[i], buffer_size, iterations, i);
    }
...
void consumerWrapper(std::vector<SharedData *> data_ptrs, std::size_t buffer_size, int num_threads)
{
    if (num_threads == 1)
    {
        consumer_avx512(data_ptrs[0], buffer_size, 0);
        return;
    }
    ...
    {
        // Create and store thread in vector
        threads.emplace_back(consumer, data_ptrs[i], buffer_size, i);
    }
...
int main(int argc, char **argv)
{
    ...
    if (pid == 0)
    {
        ...
        consumerWrapper(data_ptrs, buffer_size,
                        num_threads);  // Use the first shared memory region for simplicity
        exit(0);
    }
    else
    {
        // Parent process (producer)
        producerWrapper(data_ptrs, buffer_size, NUM_ITERS / (std::size_t)num_threads,
                        num_threads);  // Use the first shared memory region for simplicity
        wait(nullptr);
    }
