...
MODULE_DESCRIPTION("Map wc mem to userspace.");
MODULE_VERSION("0.1");
...
static int device_op_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	printk(KERN_INFO "wc ram mmap successful\n");
	return 0;
fail:
	buffer_destroy(&client->buffer);
	return ret;
...
static int __init wc_ram_init(void)
{
	printk(KERN_INFO "WC ram module loaded\n");

	cdev_major = register_chrdev(0, "wc_ram", &device_ops);
	if (cdev_major < 0)
		return cdev_major;
...
static void __exit wc_ram_exit(void)
{
	unregister_chrdev(cdev_major, "wc_ram");

	printk(KERN_INFO "wc ram module unloaded\n");
}

module_init(wc_ram_init);
module_exit(wc_ram_exit);
